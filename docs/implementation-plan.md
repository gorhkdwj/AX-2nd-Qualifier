...
- 검증: 더미 픽스처로 정량 검증하고, 로컬 Codex CLI에서 실제 설치·실행·시연한 뒤, S8 패키징 전 S7.5 확장 검증과 S7.7 실제 페이지형 합성 더미 검증으로 재현성 패키지를 보존한다.
...
| S8 | 패키징·제출 준비 | `submission.zip` 구조 요건 충족 | 구조 체크리스트, 비밀정보·크롤러 없음 확인 | S7.7 |
...
6. **정합성**: requirements-contract ↔ SKILL/스크립트 동작 ↔ README ↔ 질문 5문항 사이에 모순이 없어야 한다.
7. **안전·규정**: 크롤러·비밀정보 없음, 로그 원본 무편집, 근거는 공개 URL·확인일과 함께 기록한다.
