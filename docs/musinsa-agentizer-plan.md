# 무신사 문제 2 · 상품 데이터 에이전트화 변환기 — 구현 계획
...
- MVP 카테고리: **아우터 + 상의 2개**. taxonomy를 데이터 주도로 설계해 이후 카테고리 확장 용이.
...
**S1 · 계약·기획 고정 (구현 전 필수, CLAUDE.md 5절)**
- `docs/requirements-contract.md` 채우기: 입력(상품 상세페이지 비정형 텍스트/BYO), 출력(정규화 JSON 스키마), 주요 지표(속성별 precision/recall·중복감지 정확도 정의), 상태·오류 판정, 누락·모호 속성 처리 규칙.
- `docs/project-plan.md`·`docs/implementation-plan.md` 무신사·문제2 확정으로 갱신(S1 선정 완료 반영).
- `docs/company-selection.md` 최종 확정 체크박스 갱신, `Decisionlog.md`에 D-007(2차 도전 기업·문제 확정) 기록.
- `docs/references/`에 근거 정리: 인터뷰 URL + 핵심 인용(원본 영상 대조), 공개 사실(뉴스룸 등) 출처·확인일.
...
- 공개 무신사 상품페이지 3~5건(BYO, 출처 기록)으로 sanity check.
