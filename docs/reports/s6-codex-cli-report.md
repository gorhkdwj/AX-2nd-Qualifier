...
- 생성 출력: `out/s6-codex-output-outer-down-layerpiece.json`, `out/s6-codex-query-answer.txt`
...
## 변환 시연
대상 fixture:
- `tests/fixtures/evaluation/source_inputs.json`의 `outer_down_layerpiece`
...
검증:
```powershell
python src\skills\product-agentizer\scripts\validate.py out\s6-codex-output-outer-down-layerpiece.json --pretty
```
