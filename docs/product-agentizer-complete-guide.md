...
└── fixtures/
    ...
    ├── expanded_dummy/
    ├── full_page_dummy/
    ├── full_page_codex_smoke20/
    ├── full_page_codex_subset/
    ├── real_sanity/
    └── size_info_patterns/

tools/
├── generate_expanded_validation_fixtures.py
├── generate_full_page_dummy_fixtures.py
├── generate_size_info_pattern_fixtures.py
├── run_expanded_validation.py
├── run_full_page_codex_smoke20_cli.py
├── run_full_page_dummy_validation.py
├── run_s7_7_dedup_cross_category_recheck.py
├── run_size_info_pattern_validation.py
└── save_log.py
...
├── post-submission/
│   └── claude-cross-validation-prompt.md
└── reports/
    ├── s5-base-evaluation-results.json
    ├── s5-evaluation-report.md
    ├── s6-codex-cli-report.md
    ├── s7-7-dedup-cross-category-recheck-report.md
    ├── s7-7-dedup-cross-category-recheck-results.json
    ├── s7-7-full-page-dummy-validation-report.md
    ├── s7-7-full-page-dummy-validation-results.json
    ├── s7-8-size-info-coverage-report.md
    ├── s7-8-size-info-coverage-results.json
    ├── s7-expanded-validation-report.md
    ├── s7-expanded-validation-results.json
    ├── s8-total-validation-evaluation-report.md
    └── s9-cross-validation-report.md
```
...
| `tools/generate_full_page_dummy_fixtures.py` | S7.7 실제 페이지형 합성 더미 fixture 재생성 |
| `tools/generate_size_info_pattern_fixtures.py` | S7.8 size_info 표기 패턴 fixture 재생성 |
| `tools/run_expanded_validation.py` | S7.5 전체 검증 실행 및 결과 스냅샷 저장 |
| `tools/run_full_page_codex_smoke20_cli.py` | S7.7/S7.8 격리 workspace에서 Codex CLI actual 생성 |
| `tools/run_full_page_dummy_validation.py` | S7.7 실제 페이지형 합성 검증 실행 및 결과 스냅샷 저장 |
| `tools/run_s7_7_dedup_cross_category_recheck.py` | S7.7 전체 상품쌍 dedup cross-category 재계산 검증 |
| `tools/run_size_info_pattern_validation.py` | S7.8 size_info 패턴 검증 실행 및 결과 스냅샷 저장 |
...
| 필드 | 의미 |
|---|---|
| `schema_version` | 현재 스키마 버전. `0.2.0` 고정 |
| `source` | 입력 출처와 입력 방식 |
| `product` | 구조화된 상품 속성 |
...
```json
{
  ...
  "category": "outer",
  "subcategory": "vest",
  "detail_type": "lightweight_padding_vest",
  "materials": [],
  "fit": ["relaxed"],
  "colors": ["khaki"],
  "seasons": ["fall", "winter"],
...
| `detail_type` | 실제 몰 세부 유형 | `lightweight_padding_vest`, `short_sleeve_tshirt` |
...
```json
[
  {"schema_version": "0.2.0", "source": {}, "product": {}, "agent_descriptor": {}, "quality": {}}
]
```
...
이 프로젝트에서는 점수 0.78 이상을 high-confidence duplicate로 봅니다. S7.5에서는 합성 fixture 기준으로 서로 다른 category 사이에서 이런 high-confidence false duplicate가 0건인지 확인했습니다. 이후 S7.7에서는 300건 전체 상품쌍 44,850개를 재계산해 high-confidence cross-category false duplicate 0건을 다시 확인했습니다.
...
| 속성 micro precision | 98.68% |
| 속성 micro recall | 89.29% |
...
| micro precision | 97.93% | 통과, 목표 95% 이상 |
| micro recall | 95.95% | 통과, 목표 85% 이상 |
...
| quality.missing_fields | 75.00% | 100.00% | 9 | 3 | 0 |
...
- `quality.missing_fields`는 recall 100%이지만 precision 75.00%입니다. 즉 expected가 요구한 누락 표시는 모두 잡았지만, Codex가 더 보수적으로 누락 필드를 추가 표시했습니다.
...
| quality.missing_fields | 21.62% | 100.00% | 8 | 29 | 0 |
...
### 17.5 S7.7 실제 페이지형 합성 더미 300건과 Codex subset 50건

S7.7은 실제 상품 페이지 원문을 저장하지 않고, 실제 페이지의 정보 밀도와 노이즈 구조를 모사한 합성 상세페이지형 더미데이터로 운영형 입력 대응력을 검증한 단계입니다. 전체 합성 기준선은 300건이며, 이 중 대표 50건은 실제 Codex CLI actual을 생성해 expected를 보지 않는 격리 실행 결과로 검증했습니다.
...
300건 self-check는 expected와 reference actual이 같은 기준으로 생성되었는지, schema와 evaluator와 dedup label이 서로 일관되는지 확인하는 검증입니다. 이 300건 self-check 결과는 schema-valid 300/300, micro precision/recall 100.00%, detail_type precision/recall 100.00%, dedup accuracy 100.00%입니다. 다만 이 수치는 blind extraction 성능이 아니라 fixture·평가도구 정합성 검증으로 해석해야 합니다.
...
패키징 전 최종 보강으로 `tools/run_s7_7_dedup_cross_category_recheck.py`를 실행해 300건 전체 상품쌍 44,850개를 처음부터 다시 채점했습니다. 결과는 score >= 0.45 후보 2,788건, score >= 0.78 high-confidence 후보 23건, high-confidence cross-category false duplicate 0건이며, 저장된 S7.7 결과와 일치했습니다.
...
### 18.3 실제 페이지형 합성 검증 실행

S7.7 fixture는 아래 명령으로 재생성할 수 있습니다.

```powershell
python tools\generate_full_page_dummy_fixtures.py
```

실제 Codex CLI smoke20 actual 또는 50건 subset actual을 다시 만들려면 아래 명령을 사용합니다. 모델 실행 결과는 시점과 모델 상태에 따라 달라질 수 있으므로, 제출 기준 actual은 `tests/fixtures/full_page_codex_smoke20/actual_products.json`과 `tests/fixtures/full_page_codex_subset/actual_products.json`에 보존한 값을 기준으로 봅니다.
...
S7.7 평가 결과는 아래 명령으로 재생성합니다.

```powershell
python tools\run_full_page_dummy_validation.py
```

패키징 전 dedup cross-category 재계산 검증은 아래 명령으로 재실행합니다.

```powershell
python tools\run_s7_7_dedup_cross_category_recheck.py
```
...
docs/reports/s7-7-dedup-cross-category-recheck-results.json
docs/reports/s7-7-dedup-cross-category-recheck-report.md
```

### 18.4 size_info 패턴 검증 실행
...
- worktree에서 작업한 세션 로그는 해당 worktree 기준 `logs/`에 생길 수 있으므로, 최종 패키징 전에는 `logs/worktrees/<worktree-name>/` 아래로 원본 복사해 source-separated 구조로 보존합니다.
...
| S7.5 Codex subset micro precision | 통과, 97.93% |
| S7.5 Codex subset micro recall | 통과, 95.95% |
...
| S7.7 실제 페이지형 합성 전체 expected/reference schema-valid | 통과, 300/300 |
| S7.7 실제 페이지형 합성 전체 self-check micro precision/recall | 통과, 100.00% / 100.00% |
...
| S7.7 dedup cross-category 재계산 | 통과, 44,850쌍 재채점·고확신 교차 카테고리 오탐 0건 |
...
```json
{
  ...
  "product": {
    ...
    "category": "outer",
    "subcategory": "jacket",
    "detail_type": "suit_blazer_jacket",
    "materials": [
      {
        "part": "shell",
        "name": "polyester",
