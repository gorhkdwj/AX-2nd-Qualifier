...
### D-025 · dedup 가중치·임계값을 MVP 휴리스틱 baseline으로 확정
**상황**
- `dedup.py`는 상품쌍의 category, subcategory, detail_type, materials, colors, fit, seasons, tpo_tags, care, title 유사도를 가중 합산해 중복 후보 점수(0~1.0)를 내고, 0.78/0.55 임계값으로 `duplicate`/`possible_duplicate`/`distinct`를 판정한다.
- 이 가중치와 임계값은 실제 무신사 운영 데이터로 학습·튜닝한 값이 아니라 도메인 직관으로 정한 초기값인데, 이 성격이 Worklog(W-033)와 docs 여러 곳에만 있고 Decisionlog에는 전용 항목이 없어 "데이터 처리 방식" 결정으로서의 근거가 한곳에 고정되어 있지 않았다.

**검토한 선택지**
- (A) 가중치·임계값을 운영 데이터로 튜닝한 확정 수치로 제시한다.
- (B) 라벨링된 상품쌍 없이 설명 가능한 휴리스틱 baseline임을 명시하고, 운영 전환 시 조정 대상으로 문서화한다.

**결정**
- (B)를 채택한다. 현재 값은 MVP 휴리스틱 baseline이며, 제출물(README)·계약서·complete-guide·validation-plan에 "운영 튜닝값 아님"을 고지한다. 실제 가중치는 `dedup.py`의 `score_pair`(합 1.00)를 단일 출처로 하고 문서 표는 이에 맞춘다.

**근거**
- 과제 규정상 출처 없는 숫자·확인 불가능한 성능 주장은 근거로 인정되지 않으므로, 운영 데이터 없이 튜닝값이라 주장하면 과장이 된다. 설명 가능한 baseline으로 한정하는 편이 정직하고 검증 가능하다.

**영향**
- README·submission-questions·requirements-contract·product-agentizer-complete-guide·validation-plan이 dedup 수치를 휴리스틱으로 일관되게 서술한다. complete-guide의 가중치 표를 실제 구현(1.00 합)과 일치시켰다.

**재검토 조건**
- 사람이 라벨링한 실제(또는 공개) 상품쌍 데이터셋을 확보해 precision/recall·오탐/미탐 기준으로 가중치·임계값을 재조정할 수 있게 되면 재검토한다.
