...
5. 소재와 혼용률은 엄격하게 처리합니다.
   ...
   - 소재 항목 중 하나라도 `part: "unknown"`이면 `quality.missing_fields`에 `material_part`를 반드시 추가합니다. 반대로 모든 소재의 `part`가 확실히 알려져 있으면 `material_part`를 `missing_fields`에 넣지 않습니다(허위 누락 표시 금지).
...
`schema_version: "0.2.0"`에서 `size_info`는 문자열 배열입니다. 하지만 각 문자열은 에이전트가 검색, 필터, 설명에 재사용하기 쉬운 최소 의미 단위여야 합니다.

- 판매 가능한 사이즈 옵션은 문장 전체를 하나로 보존하지 말고 개별 옵션 값으로 나눕니다.
  ...
  - `FREE`, `ONE SIZE`, `OS`처럼 단일 옵션이면 하나의 항목으로 보존합니다.
- `사이즈 옵션`, `옵션`, `SIZE`, `Size`, `사이즈` 뒤에 쉼표, 슬래시, 세로줄, 공백으로 나열된 값이 있으면 각 값을 별도 `size_info` 항목으로 만듭니다.
- 문자와 숫자가 결합된 옵션(`M(95)`, `L-100`)은 결합된 표기 하나를 한 항목으로 보존합니다. 괄호 안 숫자를 임의로 분리하지 않습니다.
- 실측표는 무리하게 치수명별로 쪼개지 말고 사이즈별 행 단위로 보존합니다.
  - `M 총장 68cm 어깨 52cm 가슴 60cm 소매 59cm`
  - `L 총장 70cm 어깨 54cm 가슴 62cm 소매 60cm`
- 표 형태 실측은 header와 값을 결합해 사이즈별 행으로 정리합니다.
  - 입력: `사이즈(cm) 총장 어깨 가슴 소매 / M 68 50 58 60`
  - 출력: `M 총장 68cm 어깨 50cm 가슴 58cm 소매 60cm`
- 모델 착용 정보가 명시되어 있으면 입력 구절을 유지해 보존할 수 있습니다.
  - `모델 178cm L 착용`
- 기준 상품과의 사이즈 비교 문구는 입력 구절을 유지해 보존할 수 있습니다.
  - `무신사 스탠다드 M 사이즈 티셔츠와 비슷한 사이즈`
- 후기, 배송, 쿠폰, 이벤트, 마케팅 문구는 `size_info`에 넣지 않습니다.
  - `후기 요약: 사이즈가 여유 있어요`는 구매자 반응 요약이므로 `size_info`가 아닙니다.
  - `넉넉한 오버핏`, `여유 있는 실루엣`은 가능하면 `fit` 판단 근거로 사용하고, 구매 가능한 사이즈 옵션처럼 기록하지 않습니다.
  - `사이즈 만족도 적당함 82%`, `178cm 70kg 고객은 L을 많이 선택` 같은 개인화·통계형 추천 문구는 정적 상품 사이즈가 아니므로 `size_info`에 넣지 않습니다.
- 사이즈 정보가 입력에 없으면 `size_info: []`로 두고 `quality.missing_fields`에 `size_info`를 추가합니다.
- 사이즈 관련 표현은 있으나 구조화가 애매하면 입력 구절 그대로 보존하고 `quality.ambiguous_fields`에 `size_info`를 추가합니다.
...
- `사이즈 옵션: M, L, XL` 같은 나열형 사이즈를 하나의 문자열로 남기지 않고 `M`, `L`, `XL`처럼 개별 항목으로 분리했습니다.
- 실측표는 사이즈별 행 단위로 보존했고, 후기·배송·쿠폰·이벤트 문구를 `size_info`에 섞지 않았습니다.
